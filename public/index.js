...
function makeHTML(data) {
  ...
  data.forEach((element) => {
    html += `<div>${element.username} loves ${element.meal}</div>`;
  });
  return html;
}
...
function handleSubmit(e) {
  ...
  fetch("/name", {
    ...
    .then((result) => {
      document.getElementById("error").innerHTML = "";
      count % 10 === 0 ? fetchData((page = cap + 1)) : fetchData((page = cap));
    })
    .catch((err) => {
